...
| Discord / Slack ingestion | ✅ **both live + scheduled** | Discord + Slack REST fetch → message store (generic `connections/:platform` API), with a **channel browser** (list channels grouped by category for point-and-click selection); a schedule can fetch fresh messages before each run (ADR-128). Channel **send-back** shipped; DM send + an independent background poller remain |
...
| Discord message fetch | ✅ (discord.py) | ✅ | `DiscordFetcher` over Discord REST v10 (blocking `ureq`, `--features discord`); pure normalization unit-tested; bot token encrypted; fetch persists to the message store (ADR-128). Verified live (real 401 on a bogus token). **Channel browser:** `GET /connections/discord/channels?scope_id=<guild>` lists the server's text channels grouped by category for point-and-click selection (WSP-006). Gateway/streaming + polling scheduler deferred |
