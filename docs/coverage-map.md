...
8. **v2 dashboard-parity screens (ADR-133)** — a live walk of v2 (`summarybot.app`) found whole screens the rewrite still lacks: **Coverage** (server-wide + per-channel, generalizing our WhatsApp-only view), **Prompts/Perspectives** (named templates + General/Developer/Marketing/Executive/Support), **Errors** (operational error log w/ resolve), **Feeds** (RSS/Atom output), a dedicated **Webhooks** manager, **Overview**, and (optional) RuVector Explorer / Populate. Also: add the **`regenerate`** job type (v2 has it; we deferred it) and enrich Jobs/Schedules/Summaries (perspective, continuity, prompt-template, calendar view). Parity is a floor — the rewrite already exceeds v2 on grounding, token/latency metadata, Spend, tenancy/RBAC, and OAuth Connect plugins.
