...
| Delivery | ✅ **~95%** | plugin seam ✅, webhook/Confluence/email/Google Drive ✅; Discord/Slack channel + DM send ✅ (ADR-126 PlatformDm); per-destination templates remain |
| Discord / Slack ingestion | ✅ **both live + scheduled** | Discord + Slack REST fetch → message store (generic `connections/:platform` API), with a **server + channel browser** (pick the Discord guild from the bot's servers, then its channels grouped by category — no id-pasting); a schedule can fetch fresh messages before each run (ADR-128). Channel + DM send-back shipped (ADR-126); an independent background poller remains |
...
| Unified "create summary" wizard (now / schedule / retrospective) | ✅ | Create | a 2-step wizard (What → When) on a new **Create** tab unifies all three flows: pick platform + channel/chat, then Now (range presets) / Recurring (schedule + rolling) / Past (by-week retrospective). **Summary options (v2 parity, ADR-134 gap-doc):** a Perspective/template picker (built-in perspectives + saved templates) and a Length selector (brief/detailed/comprehensive) on the Now + Recurring steps — threaded through `createSchedule`→`schedule_options.length` and applied by the runner (migration 0029); previously the wizard dropped both. Dispatches to summarize-now, createSchedule, and summarize-weeks (ADR-088/089) |
...
| Jobs view (long-running work + progress) | ✅ | Jobs | a Jobs tab lists background work with status/progress/cost (ADR-013/040). **Every long-running producer now records a job**: manual summaries (`summarization`), scheduled runs (`scheduled` — both the background scheduler and a manual trigger), retrospective by-week (`backfill`), platform message sync (`sync`, progress = stored/fetched), and wiki synthesis (`wiki_synthesis`). Previously only the retrospective backfill did; the `summarization` lifecycle existed but was test-only/dead. Deferred (intentionally not jobs): regenerate (no endpoint yet), rolling-digest finalize, and read-only/fast/best-effort ops (wiki curate/export, inline knowledge-ingest). Live streaming progress mid-run is a refinement |
...
| Discord message fetch | ✅ (discord.py) | ✅ | `DiscordFetcher` over Discord REST v10 (blocking `ureq`, `--features discord`); pure normalization unit-tested; bot token encrypted; fetch persists to the message store (ADR-128). **Verified live against a real server (44 channels, 19 readable → 59 msgs).** **Actionable fetch diagnostics (ADR-134/041):** a per-channel 403 now reads "the bot can't read this channel — grant 'View Channel' + 'Read Message History' … (Discord 50001: Missing Access)", names the channel, and the sync reports coverage (`channels_total`/`channels_failed`) with one aggregated Errors row instead of one-per-channel. **Server + channel browser:** `GET /connections/discord/servers` lists the guilds the bot is in, and `…/channels?scope_id=<guild>` lists that server's text channels grouped by category — pick both point-and-click instead of pasting ids (WSP-006). **Pre-flight accessibility (ADR-097):** the channel browser now resolves the bot's effective per-channel permissions (base + role perms, ADMINISTRATOR bypass, then `@everyone`/role/member overwrites; needs `VIEW_CHANNEL` ∧ `READ_MESSAGE_HISTORY`) and flags unreadable channels with a 🔒 badge *before* a sync — permission math unit-tested, live wiring best-effort. Gateway/streaming deferred |
...
| In-chat commands (`/summarize`, `/schedule …`) | ✅ | 🟡 | **agnostic command core built** (ADR-122 CMD-001): `domain::parse_command` (scope/range/length + schedule list/pause/resume/delete/status) + `host::execute_command` (schedule-management against the store; summarize defers). The per-platform **adapters** — Discord interactions webhook + Ed25519 verify + slash-command registration, Slack — are the remaining plumbing (need a live bot to exercise) |
...
| Identity claim / transfer (WSP-011..014) | ✅ | ✅ | a `Collision` (identity bound to another user) is resolved via `/identity/claims`: `decide_claim_route` routes self-verify→auto, same-tenant→tenant admin, cross-tenant→platform operator (ADR-119); approval rebinds the identity, audited. Stored in `identity_claims`. The self-service OAuth re-auth shortcut (WSP-012) is a follow-up |
...
8. **v2 dashboard-parity screens (ADR-133)** — a live walk of v2 (`summarybot.app`) found whole screens the rewrite still lacks. **Shipped:** ✅ **Coverage** (server-wide + per-channel, generalizing the WhatsApp-only view; summaries now persist their covered window), ✅ **Prompts/Perspectives** (named templates + General/Developer/Marketing/Executive/Support, selectable on-demand), ✅ **Errors** (operational error log w/ severity/operation/scope + resolve & bulk-resolve; `operational_errors` table, recorded at the sync soft-fail (ADR-041/097) and summarize-failure sites per ADR-031/024), ✅ **Jobs `regenerate`** (re-run a stored summary over its window with changed perspective/length; v2 had it, we'd deferred it), ✅ **§B enrichments** (Jobs: enriched record + status buckets + type/status filters + per-job details; Schedules: prompt-template/perspective/title-template/continuity; Summaries: perspective + kind facets + Calendar view; Channels: bot-accessibility badge — Slack `is_member`, Discord deferred), ✅ **§A screens** (A4 Feeds — RSS at a public `/feeds/:token`; A5 Webhooks — last delivery/status folded into Delivery; A6 Overview — dashboard home; A7 RuVector Explorer — vector-store browser; A8 Populate satisfied by existing Create→Past/retrospective/Sync), ✅ **§D in-screen actions** (D1 Jobs **retry** for scheduled jobs — pause/resume N/A in the synchronous model; D2 per-summary **Publish** + bulk publish reusing the deliverers; D3 **bulk regenerate**; D4 **Report-Issue fidelity** — severity field+filter+badge, real `window.location.href`/`navigator.userAgent` capture, paginated/filtered Issues list; GitHub-mirror linkage deferred). ADR-133 is now fully shipped. **Remaining (live-only follow-ups):** **Feeds** (RSS/Atom output), a dedicated **Webhooks** manager, **Overview**, optional RuVector Explorer / Populate; and the deeper Schedules/Summaries enrichments (continuity, title_template, calendar view). Error recording at the scheduled-run/delivery sites (host-side) is a follow-up. Parity is a floor — the rewrite already exceeds v2 on grounding, token/latency metadata, Spend, tenancy/RBAC, and OAuth Connect plugins.
