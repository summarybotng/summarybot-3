...
| A4 | **Feeds** ✅ *shipped* | RSS/Atom output of a workspace's summaries; public or private; per-channel or all; access counts. | new `Feeds` tab | `feeds` table (`id, channel_id, type, is_public, url_token, access_count`); a public `GET /feeds/:token` renderer (Atom/RSS); CRUD API. |
| A5 | **Webhooks** ✅ *shipped (folded into Delivery)* | Dedicated outbound-webhook manager: typed, enabled, last delivery + status. Today webhooks are one row-type inside Delivery. | fold into `Delivery` (a Webhooks section) or a sibling tab | surface `last_delivery`/`last_status` on `workspace_destinations`; no new table. |
| A6 | **Overview** ✅ *shipped* | Per-workspace dashboard home: counts (summaries, schedules, members), recent activity, config status. | new default tab | read-only aggregate endpoint; mostly composition of existing data. |
| A7 | **RuVector Explorer** ✅ *shipped* | Browse/inspect the vector store (embeddings) for debugging knowledge. | under `Knowledge` | read-only `GET /workspaces/:ws/knowledge/vectors` projection over stored units/embeddings. |
| A8 | **Populate** ➖ *covered by Create→Past / retrospective / Sync* | Explicit "backfill/ingest history now" trigger with progress. Our Create→Past + retrospective covers most of this; Populate is the always-available manual ingest. | under `Sources` or `Create` | reuses the `Sync`/`Backfill` jobs (ADR-013/040) — mostly a UI affordance. |
...
|---|--------|-------------|--------|
| D1 | **Jobs: Pause / Resume / Retry** | Per-job operational controls ("Job paused/resumed successfully", "Retry job created"). | 🟡 — `POST /workspaces/:ws/jobs/:id/retry` re-fires a *scheduled* job via the SummarizingScheduleRunner + a "🔁 Retry" Jobs-view action. Pause/resume are **N/A by architecture**: jobs run synchronously (no executor/queue to suspend), so there is no running process to pause — documented in `jobs.rs`. |
| D2 | **Per-summary Publish to Confluence / Push to Google Drive (+ open, unpublish, bulk)** | On-demand publish/push of a *stored* summary, plus "Open in Drive" and bulk publish/unpublish (ADR-110). | ✅ — `POST /summaries/:id/publish {destination_id}` + `POST /destinations/:id/publish {summary_ids}` (bulk) reuse the deliverers + record last-delivery; per-summary "Publish to: …" picker in the Summaries card. |
| D3 | **Bulk summary Regenerate / Publish** | Bulk regenerate + bulk publish/unpublish over selected summaries. | ✅ — `POST /summaries/bulk-regenerate {ids, perspective?, length?}` (shares `regenerate_core`) + "↻ Regenerate N" action over the filtered set; bulk publish shipped under D2. |
| D4 | **Report-Issue fidelity deltas** | v2's report carries **severity** (and the Issues list filters by it — "All Severity"), auto-captured **browser/real page URL**, and a **paginated list with status/type/severity filters**; issues also link to a public GitHub tracker (ADR-070). | ✅ — `severity` (low/medium/high/critical) field + Issues severity filter + badge; real `window.location.href` page-URL + `navigator.userAgent` browser capture; list now paginated (limit/offset + `total`) with severity filter. GitHub-tracker linkage remains deferred (we keep triage in-app; not wiring a public GitHub mirror). |
