...
| A3 | **Errors** | Operational error log (fetch/LLM/delivery failures) with type, severity, code, message, operation, resolved-flag + bulk-resolve. Distinct from the human Audit log. | new `Errors` tab | `operational_errors` table; record at the existing failure sites (fetch, summarize, deliver) — reuse `FailureClass`; `GET/PATCH /workspaces/:ws/errors`. |
